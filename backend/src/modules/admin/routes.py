...
def get_user_activity(user_id: int, skip: int = 0, limit: int = 15, db: Session = Depends(get_db)):
    return AdminController.get_user_activity(user_id, skip, limit, db)
...
def admin_search_detections(
    ...
    return AdminController.admin_search_detections(plate, source_type, date_from, date_to, verified, min_confidence, max_confidence, skip, limit, db)
...
@router.post("/users")
def admin_create_user(payload: AdminCreateUser, db: Session = Depends(get_db)):
    return AdminController.admin_create_user(payload, db)

@router.put("/users/{user_id}/role")
def admin_update_user_role(user_id: int, payload: AdminUpdateRole, db: Session = Depends(get_db)):
...
@router.get("/detections/incorrect")
def admin_incorrect_detections(skip: int = 0, limit: int = 50, db: Session = Depends(get_db)):
    return AdminController.admin_incorrect_detections(skip, limit, db)
